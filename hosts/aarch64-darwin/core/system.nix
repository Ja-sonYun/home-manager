...
###################################################################################
{
  system = {
    ...
    activationScripts.postActivation.text = ''
      ...
      if [ -f "$CERT" ]; then
        /usr/bin/security add-trusted-cert -d -p ssl -p basic \
          -k /Library/Keychains/System.keychain "$CERT" || true
      fi

      # Redirect HTTP/HTTPS traffic to mitmproxy
      PFCONF="/etc/pf.conf"
      RULE="rdr pass on lo0 proto tcp to any port {80,443} -> 127.0.0.1 port 8080"
      if ! /usr/bin/grep -Fqx "$RULE" "$PFCONF"; then
        echo "$RULE" >> "$PFCONF"
      fi
